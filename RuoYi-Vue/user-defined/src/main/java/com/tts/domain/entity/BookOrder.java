...
public class BookOrder extends BaseEntity {
    ...
    /** 预约开始时间模糊查询参数（支持年月日、年月日时、年月日时分等） */
    private String reserveStartMatch;
     ...
     * 根据预约开始时间和时长计算预约结束时间
     */
    public void calculateReserveEnd() {
        if (this.reserveStart != null && this.duration != null) {
            // 将预约时长（分钟）转换为毫秒
            long durationInMillis = this.duration * 60 * 1000;
            // 计算结束时间
            this.reserveEnd = new Date(this.reserveStart.getTime() + durationInMillis);
        }
    }
}